...
if [ -z "$wlan0_ip" ];then
    echo "wlan0 ip null"
    wlan0_ip="192.168.11.10"
    wlan0_gw="192.168.11.1"
else
    echo "wlan0_ip:$wlan0_ip"
    wlan0_gw=`get_gw_by_ip $wlan0_ip`".1"
fi
...
echo "${eth0_ip}" > /usr/local/rak/gateway-config-info/eth0_ip
echo "${eth0_gw}" > /usr/local/rak/gateway-config-info/eth0_gw
sleep 5
